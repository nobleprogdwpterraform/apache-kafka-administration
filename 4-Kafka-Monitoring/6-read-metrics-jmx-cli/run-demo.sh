...
echo "Waiting for kafka-1 Kafka listener on 9092..."
...
for i in {1..60}; do
  ...
  if docker exec kafka-1 bash -lc "ps aux | grep -E '[k]afka.Kafka' >/dev/null"; then
    ...
    if docker exec kafka-1 bash -lc "timeout 1 bash -lc '</dev/tcp/localhost/9092' >/dev/null 2>&1"; then
...
if ! docker exec kafka-1 bash -lc "unset JMX_PORT KAFKA_JMX_OPTS KAFKA_JMX_PORT; /opt/kafka/bin/kafka-topics.sh --bootstrap-server kafka-1:9092 --list >/dev/null 2>&1"; then
  echo ""
