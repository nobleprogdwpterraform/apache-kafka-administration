...
fetch() {
  docker exec kafka-1 bash -lc "
    env -u JMX_PORT -u KAFKA_JMX_PORT -u KAFKA_JMX_OPTS -u KAFKA_OPTS -u JAVA_TOOL_OPTIONS \
      /opt/kafka/bin/kafka-consumer-groups.sh --bootstrap-server kafka-1:9090 --describe --group '$GROUP' 2>&1 || true
  "
}
...
echo -e "${DIM}Consumer may have failed. Check:${RESET} docker exec kafka-1 tail -n 80 $LOG"
echo "--------------------------------------------------------------------------"
exit 0
