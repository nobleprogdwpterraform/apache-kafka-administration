...
echo "Starting consumer inside kafka-1 (topic=$TOPIC, group=$GROUP)..."
...
PID_LINE="$(docker exec kafka-1 bash -lc "pgrep -af kafka-console-consumer | head -n 1 || true")"
if [[ -n "$PID_LINE" ]]; then
...
docker exec kafka-1 bash -lc "tail -n 80 '${LOG}' 2>/dev/null || echo 'NO LOG FILE'"
exit 1
